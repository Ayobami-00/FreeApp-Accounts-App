...
import 'package:freeapp_accounts_app/widgets/addDebits.dart';
...
class _DebitsState extends State<Debits> {
        ...
        children: <Widget>[
          ...
          Row(
            ...
            children: <Widget>[
              SizedBox(width: 10.0),
              FlatButton.icon(
                    onPressed: ()=> Navigator.push(context, MaterialPageRoute(builder: (context)=>AddDebit())), 
                    icon: Icon(Icons.add), 
                    label: Text("DEBITS",style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16.0)),
                    ),
                    SizedBox(width: 10.0),
  ...
  Widget _buildBody(BuildContext context){
    return StreamBuilder<QuerySnapshot>(
      stream: Firestore.instance.collection('debits').orderBy('timestamp',descending: true)
      .snapshots(),
  ...
  Widget _buildMonthlyBody(BuildContext context,int monthNumberVal){
    return StreamBuilder<QuerySnapshot>(
      stream: Firestore.instance.collection('debits')
